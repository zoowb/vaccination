...
router.post("/index", async function (req, res, next) {
  ...
  try {
    const sql1 =
      "select distinct Hnumber, Hname " +
      "from hospital as h, person as p " +
      "where p.Ssn=? and sqrt(pow(p.x-h.x,2)+pow(p.y-h.y,2)) < 0.02;"; // 300m 차이 -> 0.03
    ...
    for (
      var i = 0;
      i < packet.length;
      i++ // 각 병원의 잔여백신 리스트 구하기
    ) {
      const sql2 =
        "SELECT V.Vnumber, Vname, Amount FROM hospital_vaccine natural join vaccine as V WHERE `Hnumber`=? and `Expiration` < ?;";
