...
router.post("/getinfo", async function (req, res, next) {
  ...
  pool.query(
    "SELECT Name, Ssn, Phone, Email, Password, Location, x, y FROM PERSON WHERE Email=?;",
    [token_res.id],
    function (err, result) {
      if (err) {
        res.send({ err: "DB 오류" });
...
router.post("/getrev", async function (req, res, next) {
  ...
  try {
    const result1 = await connection.query(
      "SELECT Rnumber, Name, Vnumber, Hnumber, Rdate FROM PERSON natural join RESERVATION WHERE `Ssn`=? and `Order`=1;",
      [ssn]
    );
    const data1 = result1[0];
    const result2 = await connection.query(
      "SELECT Rnumber, Vnumber, Hnumber, Rdate FROM PERSON natural join RESERVATION WHERE `Ssn`=? and `Order`=2;",
      [ssn]
    );
    ...
    const result3 = await connection.query(
      "SELECT Hname FROM hospital WHERE `Hnumber`=?;",
      [data1[0].Hnumber]
    );
    ...
    const result5 = await connection.query(
      "SELECT Vname FROM vaccine WHERE `Vnumber`=?;",
      [data1[0].Vnumber]
    );
...
router.post("/changerev", function (req, res, next) {
  ...
  pool.getConnection(function (err, connection) {
    var sql =
      "UPDATE RESERVATION SET Hnumber=?, Vnumber=?, Rdate=? WHERE Rnumber=?";
    connection.query(
      sql,
      [rev_hos, rev_vac, rev_date, rev_num],
      function (err, result) {
        if (err) {
          // 등록 오류
          res.send({ err: "DB 오류" });
...
router.post("/delrev", async function (req, res, next) {
  ...
  try {
    const result1 = await connection.query(
      "SELECT Rdate FROM RESERVATION WHERE `Rnumber`=?;",
      [rev1_num]
    );
    const data1 = result1[0];
    const result2 = await connection.query(
    ...
    await connection.query(
      "delete from `reservation` where `Rnumber` in(?,?)",
      [rev1_num, rev2_num]
    );
    res.send({ ok: true });
  } catch (err) {
