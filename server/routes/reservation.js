 ...
 * === client-input ===
 * jwtToken = 사용자 정보 jwt토큰 (로그인에서 생성된 토큰)
 * rev_date = 예약 날짜&시간 (ex. '2021-10-30 09:30:00')
 * rev_hos = 예약 병원 아이디
 * rev_date = 1차예약 날짜
 *
 * === server-return ===
...
router.post('/register', async function (req, res, next) {
    ...
    try {
        ...
        const sql1 = "select HV.Vnumber, Vname, Amount from hospital_vaccine as HV, " + 
            ...
            "HV.`Expiration` < ? and HV.`Amount` > 0 and V.`AgeCont` < (select age from person where Ssn=?);";
        const result1 = await connection.query(sql1, [rev_hos, today, rev_ssn]);
        const data1 = result1[0];
        ...
        {
            err_code = 2;
            throw new Error("잔여 백신이 없습니다.");
        }
        const rev_vacid = rev_vac.Vnumber;

        let rev_date2 = (new Date()).setDate(rev_date.getDate() + 28);
        ...
        if(rev_vac.Amount > 1) // 잔여백신이 2 이상이면 1 감소시킴. 아니면 삭제
            await connection.query("update hospital_vaccine SET Amount=Amount-1 WHERE Hnumber=?, Vnumber=?", [rev_hos, rev_vacid]);
        else
            await connection.query("delete from hospital_vaccine WHERE Hnumber=?, Vnumber=?", [rev_hos, rev_vacid]);

        res.send({ rev_vacname : rev_vac.Vname });
